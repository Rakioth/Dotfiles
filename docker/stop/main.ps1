...
if ($args.Count -gt 0) {
    ...
    $options = docker ps --format "{{.Names}}" | Sort-Object -Unique
    ...
    $selected        = Gum-Filter -Header "🐳 Select the $containersLabel to stop: " -Options $options
}
...
$selected | Sort-Object -Unique | ForEach-Object {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Container successfully stopped: $containerLabel"
    } else {
